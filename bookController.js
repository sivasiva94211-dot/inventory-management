...
const bookController = {
  addBook: async (req, res) => {
    ...
    const { id } = req.user;
    try {
      const checkIsAval = await Book.findOne({ title: title, user: id });
      if (checkIsAval) {
        return res.status(200).send({ msg: "Book already exists" });
      }
  ...
  deleteBook: async (req, res) => {
    ...
    try {
      ...
      await Book.findOneAndDelete({ _id: bookId, user: id });
